...
<template>
  ...
  >
    <div
      v-if="visible"
      class="fixed bottom-4 right-4 z-50 w-[calc(100%-2rem)] max-w-sm rounded-lg border border-gray-200 bg-white p-4 shadow-xl sm:right-6 sm:bottom-6 relative"
    >
      <button
        type="button"
        aria-label="Dismiss"
      ...
      <div class="mt-3 flex items-center gap-4">
        <router-link
          to="/hope"
          class="rounded-md bg-urgent px-3 py-1.5 text-sm font-semibold text-white hover:bg-red-800"
          @click="dismiss"
        >
          Learn more
        </router-link>
        <button type="button" class="text-sm text-gray-500 hover:underline" @click="dismiss">
          Dismiss
        </button>
      </div>
    </div>
